...
This document describes a focused compatibility bridge rather than a complete
SDR client. The first implementation now lives in [`client/`](../client/) and
provides a read-only connection/status milestone. The remaining interfaces are
design direction, not promises of future delivery. The public API remains
available to anyone who wants to build a richer client.

The client remains part of this source project so its API compatibility can be
tested alongside the daemon. It is a separate build component but is included
in the current combined daemon package. Later it can also be produced as a
client-only Debian package; the daemon package can depend on that package if
combined installation remains desirable.
