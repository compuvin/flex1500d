...
enum {
    REQUEST_INDEX = 2,
    ISO_PACKET_COUNT = 64,
    ISO_BUFFER_SIZE = ISO_PACKET_COUNT * FLEX1500_SAMPLE_PACKET_SIZE,
...
static void print_plan(void)
{
    ...
    flex1500_build_initialize_request(REQUEST_INDEX, command);

    puts("FLEX-1500 minimum RX initialization probe (NOT ARMED)");
    ...
    puts("  4. Send one interrupt-OUT command on endpoint 0x04:");
    puts("       opcode 1219 INITIALIZE, param1=0, param2=0.");
    puts("  5. Submit one isochronous-IN transfer to endpoint 0x82:");
    puts("       64 packets x 192 bytes, representing 64 ms.");
    puts("  6. Report packet and signed-16-bit I/Q statistics.");
    puts("  7. Release the interface and close the device.");
    puts("Excluded: all other opcodes, endpoint 0x01, endpoint 0x83,");
    puts("control transfers, reset, configuration changes, and alt settings.");
    print_bytes("Exact INITIALIZE command", command, sizeof(command));
...
static bool is_armed(int argc, char **argv)
{
    ...
    if (argc == 2 && strcmp(argv[1], "--help") == 0) {
        printf("Usage: %s [--help]\n", argv[0]);
        printf("       %s --execute-approved-min-rx-init-probe\n", argv[0]);
        exit(EXIT_SUCCESS);
    }
    if (argc == 2 &&
        strcmp(argv[1], "--execute-approved-min-rx-init-probe") == 0) {
        return true;
    }
...
static void update_stats(sample_stats *stats, const uint8_t *data,
                         unsigned int length)
{
    for (unsigned int index = 0; index < length; ++index) {
        if (data[index] != 0) ++stats->nonzero_bytes;
    }
...
static void inspect_samples(struct libusb_transfer *transfer,
                            sample_stats *stats)
{
    ...
    for (int packet_index = 0; packet_index < transfer->num_iso_packets;
        ...
        if (!printed_payload) {
            ...
            print_bytes("First sample payload", data, shown);
...
static int run_probe(void)
{
    ...
    struct libusb_transfer *sample_transfer = NULL;
    completion_state sample_state = {0};
    uint8_t command[FLEX1500_COMMAND_PACKET_SIZE];
    ...
    if (result != 0) {
        fputs(result == 1 ?
                  "Refusing probe: interface 3 has an active kernel driver.\n" :
                  "Cannot verify kernel-driver state.\n",
              stderr);
        goto cleanup;
    }
    ...
    flex1500_build_initialize_request(REQUEST_INDEX, command);
    {
        ...
        result = libusb_interrupt_transfer(
            handle, FLEX1500_EP_COMMAND_OUT, command, sizeof(command),
            &transferred, TRANSFER_TIMEOUT_MS);
        print_bytes("INITIALIZE command sent", command, sizeof(command));
        printf("command status: %d, bytes: %d\n", result, transferred);
        if (result != LIBUSB_SUCCESS ||
            transferred != FLEX1500_COMMAND_PACKET_SIZE) {
            fputs("INITIALIZE command did not complete in full.\n", stderr);
            goto cleanup;
        }
    }
    ...
    if (!flex1500_rx_probe_transfer_allowed(FLEX1500_TRANSFER_ISOCHRONOUS,
        ...
        fputs("Safety policy rejected sample endpoint 0x82.\n", stderr);
        goto cleanup;
    }

    sample_transfer = libusb_alloc_transfer(ISO_PACKET_COUNT);
    sample_buffer = calloc(1, ISO_BUFFER_SIZE);
    if (sample_transfer == NULL || sample_buffer == NULL) {
        fputs("Unable to allocate sample-IN transfer.\n", stderr);
        goto cleanup;
    }
    libusb_fill_iso_transfer(
        sample_transfer, handle, FLEX1500_EP_SAMPLE_IN, sample_buffer,
        ISO_BUFFER_SIZE, ISO_PACKET_COUNT, transfer_complete, &sample_state,
        TRANSFER_TIMEOUT_MS);
    libusb_set_iso_packet_lengths(sample_transfer,
                                  FLEX1500_SAMPLE_PACKET_SIZE);
    result = libusb_submit_transfer(sample_transfer);
    if (result != LIBUSB_SUCCESS) {
        fprintf(stderr, "Cannot submit sample-IN transfer: %s\n",
                libusb_error_name(result));
        goto cleanup;
    }
    if (wait_for_transfer(context, &sample_state, sample_transfer) !=
        EXIT_SUCCESS) {
        goto cleanup;
    }

    printf("sample transfer status: %d\n", sample_transfer->status);
    inspect_samples(sample_transfer, &stats);
...
cleanup:
    if (sample_transfer != NULL) libusb_free_transfer(sample_transfer);
    free(sample_buffer);
    if (claimed) {
        result =
