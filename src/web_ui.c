...
"<label>DSP bandwidth<br><output id=\"bandwidth\">6000 Hz</output></label>\n"
...
"<p class=\"warn\">Tuning or changing mode stops audio. CW uses a 700 Hz receive tone.</p></fieldset>\n"
...
"document.querySelector('#tune').onclick=async()=>{await stopAudio();const hz=document.querySelector('#frequency').value;try{const r=await fetch('/v1/radio/frequency/'+hz,{method:'PUT'});const body=await r.json();if(!r.ok)throw new Error(JSON.stringify(body));show(body);await refresh()}catch(e){show('Tune error: '+e)}};\n";
...
"const reader=response.body.getReader();let buffered=new Uint8Array(0),dc=0,lp=0,env=1,phase=0,pi=0,pq=0,havePrev=false,settle=2400,frames=0;\n"
"const nt=65,hi=new Float32Array(nt),hq=new Float32Array(nt),ci=new Float32Array(nt),cq=new Float32Array(nt);let hp=0;if(mode==='usb'||mode==='lsb'||mode==='cw'){const cutoff=mode==='cw'?250:1350,center=mode==='usb'?1450:mode==='lsb'?-1450:0,mid=(nt-1)/2;let sum=0;for(let t=0;t<nt;t++){const k=t-mid,w=.54-.46*Math.cos(2*Math.PI*t/(nt-1)),b=(k===0?2*cutoff/48000:Math.sin(2*Math.PI*cutoff*k/48000)/(Math.PI*k))*w;ci[t]=b*Math.cos(2*Math.PI*center*k/48000);cq[t]=b*Math.sin(2*Math.PI*center*k/48000);sum+=b}for(let t=0;t<nt;t++){ci[t]/=sum;cq[t]/=sum}}\n";
...
"for(let n=0;n<count;n++){const i=v.getFloat32(20+n*8,true),q=v.getFloat32(24+n*8,true);let x=0;if(mode==='am'){x=Math.hypot(i,q)}else if(mode==='fm'){if(havePrev)x=Math.atan2(pi*q-pq*i,pi*i+pq*q);pi=i;pq=q;havePrev=true}else{hi[hp]=i;hq[hp]=q;let fi=0,fq=0,p=hp;for(let t=0;t<nt;t++){fi+=hi[p]*ci[t]-hq[p]*cq[t];fq+=hi[p]*cq[t]+hq[p]*ci[t];p=p===0?nt-1:p-1}hp=(hp+1)%nt;if(mode==='cw'){phase+=2*Math.PI*700/48000;x=fi*Math.cos(phase)-fq*Math.sin(phase)}else{x=fi}}if(phase>Math.PI)phase-=2*Math.PI;dc+=.0005*(x-dc);x-=dc;lp+=.25*(x-lp);env=Math.max(Math.abs(lp),env*.9995);audio[n]=settle-->0?0:Math.max(-.8,Math.min(.8,.24*lp/(env+1e-6)))}\n"
