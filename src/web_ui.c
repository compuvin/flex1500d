...
"<script>\n"
"const statusBox=document.querySelector('#status');let aborter=null,context=null,rxOutput=null,underruns=0,tuneLease=null,tuneTimer=null,txLease=null,txTimer=null,txContext=null,txStream=null,txNode=null,txUpload=Promise.resolve(),txKeyed=false,tuneAudioMuted=false;\n"
...
"const node=new AudioWorkletNode(context,'f15-audio',{outputChannelCount:[1]});underruns=0;node.port.onmessage=e=>{if(e.data&&e.data.underruns!==undefined)underruns=e.data.underruns};rxOutput=context.createGain();rxOutput.gain.value=tuneAudioMuted?0:1;node.connect(rxOutput);rxOutput.connect(context.destination);await context.resume();\n"
...
"async function stopMic(){if(txTimer)clearInterval(txTimer);txTimer=null;if(txLease!==null&&txKeyed){try{await api('/v1/tx/ptt/stop','PUT',txLease)}catch(e){show('Mic unkey error: '+e)}}txKeyed=false;if(txNode){txNode.disconnect();txNode.onaudioprocess=null}txNode=null;if(txStream)for(const t of txStream.getTracks())t.stop();txStream=null;if(txContext)await txContext.close();txContext=null;try{await txUpload}catch(e){}if(txLease!==null){try{await api('/v1/tx/sessions/current','DELETE',txLease)}catch(e){}}txLease=null}\n"
...
"document.querySelector('#micStart').onclick=async()=>{try{if(!window.isSecureContext||!navigator.mediaDevices)throw new Error('Microphone capture requires localhost or HTTPS');await stopMic();await stopAudio();const mode=document.querySelector('#mode').value.toLowerCase();if(mode!=='usb'&&mode!=='lsb')throw new Error('Computer microphone TX supports USB or LSB only');await applyTxSettings();const drive=Number(document.querySelector('#txdrive').value);const j=await api('/v1/tx/sessions','POST',null,{mode,source:'audio',drive_percent:drive,sample_rate:48000,sample_format:'s16le',channels:1});txLease=j.lease;txStream=await navigator.mediaDevices.getUserMedia({audio:{channelCount:1,echoCancellation:false,noiseSuppression:false,autoGainControl:false}});txContext=new AudioContext({sampleRate:48000});if(txContext.sampleRate!==48000)throw new Error('Browser microphone rate is '+txContext.sampleRate+', expected 48000');const source=txContext.createMediaStreamSource(txStream);txNode=txContext.createScriptProcessor(4096,1,1);const silent=txContext.createGain();silent.gain.value=0;source.connect(txNode);txNode.connect(silent);silent.connect(txContext.destination);txNode.onaudioprocess=e=>{if(txLease===null)return;const f=e.inputBuffer.getChannelData(0),b=new ArrayBuffer(f.length*2),v=new DataView(b);for(let n=0;n<f.length;n++)v.setInt16(n*2,Math.max(-32768,Math.min(32767,Math.round(f[n]*32767))),true);txUpload=txUpload.then(()=>api('/v1/tx/audio','POST',txLease,b)).catch(error=>{show('Mic upload error: '+error);setTimeout(()=>stopMic(),0)})};await txContext.resume();await new Promise(resolve=>setTimeout(resolve,900));await txUpload;await api('/v1/tx/ptt/start','PUT',txLease);txKeyed=true;txTimer=setInterval(()=>api('/v1/tx/sessions/keepalive','PUT',txLease).catch(e=>show('TX keepalive error: '+e)),5000);show('Computer microphone transmitting: '+mode.toUpperCase()+' '+drive+'%')}catch(e){show('Mic TX start error: '+e);await stopMic()}};\n"
...
const char *flex1500_web_ui(size_t *length)
{
    static char page[sizeof(page_part1a) + sizeof(page_part1b) +
                     sizeof(page_part2) + sizeof(page_part3) +
                     sizeof(page_part4a) + sizeof(page_part4b) - 5];
